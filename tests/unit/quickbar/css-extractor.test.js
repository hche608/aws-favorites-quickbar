...
const { teardownDOM, createMockElement } = require('../../helpers/dom-helpers');
...
describe('CSS Extractor', () => {
  ...
  describe('extractAWSFavoriteClasses', () => {
    it('should extract CSS classes from native favorites when present', () => {
      ...
      const anchor = createMockElement('a', { className: 'globalNav-1215 globalNav-1284 globalNav-1285' });
      const mainContainer = createMockElement('div', { className: 'globalNav-1286' });
      const iconWrapper = createMockElement('div', { className: 'globalNav-1290 globalNav-1288 globalNav-1289' });
      const icon = createMockElement('img', { className: 'globalNav-1291 globalNav-1293 globalNav-1288 globalNav-1289' });
      ...
      const result = window.AWSFavoritesQuickbar.extractAWSFavoriteClasses();
  ...
  describe('waitForNativeFavorites', () => {
    it('should resolve immediately if CSS classes are already present', async () => {
      ...
      const result = await window.AWSFavoritesQuickbar.waitForNativeFavorites(quickbar, 1000);
    ...
    it('should wait for MutationObserver to detect native favorites', async () => {
      ...
      // Start waiting
      const promise = window.AWSFavoritesQuickbar.waitForNativeFavorites(quickbar, 1000);
    ...
    it('should timeout and return null if native favorites never appear', async () => {
      ...
      const result = await window.AWSFavoritesQuickbar.waitForNativeFavorites(quickbar, 100);
