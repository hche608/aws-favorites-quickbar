...
describe('Browser API Compatibility', () => {
  ...
  describe('Property 1: Storage operations produce identical results across browsers', () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (favorites) => {
            // Test Chrome API
            setupChromeMocks();
            await (global as any).chrome.storage.sync.set({ userFavorites: favorites });
            const chromeResult = await (global as any).chrome.storage.sync.get(['userFavorites']);
            clearAllBrowserMocks();

            // Test Firefox API
            setupFirefoxMocks();
            await (global as any).browser.storage.sync.set({ userFavorites: favorites });
            const firefoxResult = await (global as any).browser.storage.sync.get(['userFavorites']);
            clearAllBrowserMocks();
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (cachedData) => {
            // Test Chrome API
            setupChromeMocks();
            await (global as any).chrome.storage.local.set({ cachedServices: cachedData });
            const chromeResult = await (global as any).chrome.storage.local.get(['cachedServices']);
            clearAllBrowserMocks();

            // Test Firefox API
            setupFirefoxMocks();
            await (global as any).browser.storage.local.set({ cachedServices: cachedData });
            const firefoxResult = await (global as any).browser.storage.local.get([
              'cachedServices'
            ]);
            clearAllBrowserMocks();
    ...
    test('save and load maxServices produces identical results', async () => {
      await fc.assert(
        fc.asyncProperty(fc.integer({ min: 1, max: 50 }), async (maxServices) => {
          // Test Chrome API
          setupChromeMocks();
          await (global as any).chrome.storage.sync.set({ maxServices });
          const chromeResult = await (global as any).chrome.storage.sync.get(['maxServices']);
          clearAllBrowserMocks();

          // Test Firefox API
          setupFirefoxMocks();
          await (global as any).browser.storage.sync.set({ maxServices });
          const firefoxResult = await (global as any).browser.storage.sync.get(['maxServices']);
          clearAllBrowserMocks();
    ...
    test('update operation (read-modify-write) produces identical results', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            initial: fc.array(fc.string({ minLength: 1, maxLength: 20 }), {
              minLength: 1,
              maxLength: 10
            }),
          ...
          async ({ initial, toAdd }) => {
            // Test Chrome API
            setupChromeMocks();
            await (global as any).chrome.storage.sync.set({ userFavorites: initial });
            const chromeRead = await (global as any).chrome.storage.sync.get(['userFavorites']);
            const chromeUpdated = [...chromeRead.userFavorites, toAdd];
            await (global as any).chrome.storage.sync.set({ userFavorites: chromeUpdated });
            const chromeFinal = await (global as any).chrome.storage.sync.get(['userFavorites']);
            clearAllBrowserMocks();

            // Test Firefox API
            setupFirefoxMocks();
            await (global as any).browser.storage.sync.set({ userFavorites: initial });
            const firefoxRead = await (global as any).browser.storage.sync.get(['userFavorites']);
            const firefoxUpdated = [...firefoxRead.userFavorites, toAdd];
            await (global as any).browser.storage.sync.set({ userFavorites: firefoxUpdated });
            const firefoxFinal = await (global as any).browser.storage.sync.get(['userFavorites']);
            clearAllBrowserMocks();
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ initial, toRemove }) => {
            ...
            await (global as any).chrome.storage.sync.set({ userFavorites: initial });
            const chromeRead = await (global as any).chrome.storage.sync.get(['userFavorites']);
            const chromeFiltered = chromeRead.userFavorites.filter((id: string) => id !== toRemove);
            await (global as any).chrome.storage.sync.set({ userFavorites: chromeFiltered });
            const chromeFinal = await (global as any).chrome.storage.sync.get(['userFavorites']);
            clearAllBrowserMocks();
            ...
            await (global as any).browser.storage.sync.set({ userFavorites: initial });
            const firefoxRead = await (global as any).browser.storage.sync.get(['userFavorites']);
            const firefoxFiltered = firefoxRead.userFavorites.filter(
              (id: string) => id !== toRemove
            );
            await (global as any).browser.storage.sync.set({ userFavorites: firefoxFiltered });
            const firefoxFinal = await (global as any).browser.storage.sync.get(['userFavorites']);
            clearAllBrowserMocks();
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (favorites) => {
            ...
            setupChromeMocks();
            await (global as any).chrome.storage.sync.set({ userFavorites: favorites });
            await (global as any).chrome.storage.sync.clear();
            const chromeResult = await (global as any).chrome.storage.sync.get(['userFavorites']);
            clearAllBrowserMocks();
            ...
            setupFirefoxMocks();
            await (global as any).browser.storage.sync.set({ userFavorites: favorites });
            await (global as any).browser.storage.sync.clear();
            const firefoxResult = await (global as any).browser.storage.sync.get(['userFavorites']);
            clearAllBrowserMocks();
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ key, value }) => {
            // Test Chrome API
            setupChromeMocks();
            await (global as any).chrome.storage.sync.set({ [key]: value });
            const chromeResult = await (global as any).chrome.storage.sync.get(key);
            expect(chromeResult[key]).toEqual(value);
            clearAllBrowserMocks();

            // Test Firefox API
            setupFirefoxMocks();
            await (global as any).browser.storage.sync.set({ [key]: value });
            const firefoxResult = await (global as any).browser.storage.sync.get(key);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (data) => {
            // Test Chrome API
            setupChromeMocks();
            const chromeSetResult = (global as any).chrome.storage.sync.set(data);
            ...
            // Test Firefox API
            setupFirefoxMocks();
            const firefoxSetResult = (global as any).browser.storage.sync.set(data);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ key, value }) => {
            // Test Chrome API
            setupChromeMocks();
            await (global as any).chrome.storage.local.set({ [key]: value });
            const chromeResult = await (global as any).chrome.storage.local.get(key);
            expect(chromeResult[key]).toEqual(value);
            clearAllBrowserMocks();

            // Test Firefox API
            setupFirefoxMocks();
            await (global as any).browser.storage.local.set({ [key]: value });
            const firefoxResult = await (global as any).browser.storage.local.get(key);
            expect(firefoxResult[key]).toEqual(value);
            clearAllBrowserMocks();
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (message) => {
            // Test Chrome API
            setupChromeMocks();
            const chromeResult = (global as any).chrome.runtime.sendMessage(message);
            ...
            // Test Firefox API
            setupFirefoxMocks();
            const firefoxResult = (global as any).browser.runtime.sendMessage(message);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (queryInfo) => {
            // Test Chrome API
            setupChromeMocks();
            const chromeResult = (global as any).chrome.tabs.query(queryInfo);
            expect(chromeResult).toBeInstanceOf(Promise);
            ...
            // Test Firefox API
            setupFirefoxMocks();
            const firefoxResult = (global as any).browser.tabs.query(queryInfo);
            expect(firefoxResult).toBeInstanceOf(Promise);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ tabId, message }) => {
            // Test Chrome API
            setupChromeMocks();
            const chromeResult = (global as any).chrome.tabs.sendMessage(tabId, message);
            expect(chromeResult).toBeInstanceOf(Promise);
            await expect(chromeResult).resolves.toHaveProperty('success', true);
            clearAllBrowserMocks();
            ...
            // Test Firefox API
            setupFirefoxMocks();
            const firefoxResult = (global as any).browser.tabs.sendMessage(tabId, message);
            expect(firefoxResult).toBeInstanceOf(Promise);
            await expect(firefoxResult).resolves.toHaveProperty('success', true);
            clearAllBrowserMocks();
          }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ key, defaultValue }) => {
            ...
            setupChromeMocks();
            const chromeResult = await (global as any).chrome.storage.sync.get({
              [key]: defaultValue
            });
            ...
            setupFirefoxMocks();
            const firefoxResult = await (global as any).browser.storage.sync.get({
              [key]: defaultValue
            });
    ...
    test('storage.clear returns Promise in both Chrome and Firefox', async () => {
      await fc.assert(
        fc.asyncProperty(fc.constant(null), async () => {
          // Test Chrome API
          setupChromeMocks();
          await (global as any).chrome.storage.sync.set({ test: 'value' });
          const chromeClearResult = (global as any).chrome.storage.sync.clear();
          expect(chromeClearResult).toBeInstanceOf(Promise);
          await chromeClearResult;
          const chromeAfterClear = await (global as any).chrome.storage.sync.get('test');
          expect(chromeAfterClear).toEqual({});
          clearAllBrowserMocks();

          // Test Firefox API
          setupFirefoxMocks();
          await (global as any).browser.storage.sync.set({ test: 'value' });
          const firefoxClearResult = (global as any).browser.storage.sync.clear();
          expect(firefoxClearResult).toBeInstanceOf(Promise);
          await firefoxClearResult;
          const firefoxAfterClear = await (global as any).browser.storage.sync.get('test');
