/**
 * Unit tests for icon-validator.js
 * Tests icon URL validation with property-based testing
 */

const fc = require('fast-check');
const { mockImage } = require('../../helpers/mocks');
...
describe('icon-validator', () => {
  ...
  beforeEach(() => {
    ...
    // Load the module
    require('../../../src/services/icon-validator');
  ...
  describe('isValidIconUrl', () => {
    it('should return false for null or undefined', async () => {
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl(null)).toBe(false);
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl(undefined)).toBe(false);
    });

    it('should return false for non-string values', async () => {
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl(123)).toBe(false);
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl({})).toBe(false);
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl([])).toBe(false);
    });

    it('should return false for strings shorter than minimum length', async () => {
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl('short')).toBe(false);
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl('https://a.com')).toBe(false);
    });

    it('should return true for valid data URIs', async () => {
      const dataUri = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg"><circle cx="50" cy="50" r="40"/></svg>';
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl(dataUri)).toBe(true);
    });

    it('should return false for short data URIs', async () => {
      const shortDataUri = 'data:image/svg+xml,<svg></svg>';
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl(shortDataUri)).toBe(false);
    });
    ...
    it('should validate HTTPS URLs by loading them', async () => {
      ...
      const result = await window.AWSFavoritesQuickbar.isValidIconUrl(url);
    ...
    it('should return false for non-HTTPS URLs', async () => {
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl('http://example.com/icon.png')).toBe(false);
      expect(await window.AWSFavoritesQuickbar.isValidIconUrl('ftp://example.com/icon.png')).toBe(false);
    });
        ...
        await fc.assert(
          fc.asyncProperty(
            fc.oneof(
              ...
              fc.constant('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg"><circle cx="50" cy="50" r="40"/></svg>'),
              fc.constant(null),
            ...
            async (input) => {
              ...
              let result;
              try {
                result = await window.AWSFavoritesQuickbar.isValidIconUrl(input);
              } catch (error) {
                threw = true;
              }
        ...
        await fc.assert(
          fc.asyncProperty(
            fc.oneof(
              ...
              fc.string({ minLength: 20, maxLength: 50 }).filter(s => !s.startsWith('https://') && !s.startsWith('data:image/'))
            ),
            async (url) => {
              const result = await window.AWSFavoritesQuickbar.isValidIconUrl(url);
  ...
  describe('updateServiceIcons', () => {
    ...
    it('should update service icons when valid new icons are available', async () => {
      const services = [
        { id: 's3', name: 'S3', iconUrl: 'https://old.com/s3.png', consoleUrl: 'https://console.aws.amazon.com/s3' }
      ];
      const iconMap = {
        s3: 'https://new.awsstatic.com/s3-icon.png'
      };

      const updated = await window.AWSFavoritesQuickbar.updateServiceIcons(services, iconMap);
