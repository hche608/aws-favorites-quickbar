...
describe('Duplicate Service Filtering Property Tests', () => {
  ...
  describe('Property 8: Duplicate service filtering', () => {
    ...
    it('should ensure merged services have no duplicates before injection', () => {
      ...
      fc.assert(
        fc.property(
          fc.array(serviceArbitrary, { minLength: 1, maxLength: 15 }),
          (services) => {
            // Split into user and recent
            const userServices = services.filter(s => s.source === 'user' || Math.random() > 0.5).map(s => ({ ...s, source: 'user' }));
            const recentServices = services.filter(s => s.source === 'recent' || Math.random() > 0.5).map(s => ({ ...s, source: 'recent' }));

            // Merge services
            const mergedServices = window.AWSFavoritesQuickbar.mergeServices(userServices, recentServices);
          ...
          (userFavorites, recentServices) => {
            ...
            if (userFavorites.length > 0 && recentServices.length > 0) {
              ...
              const serviceWithDuplicateId = mergedServices.find(
                s => s.id.toLowerCase() === duplicateId
              );
    ...
    it('should handle case-insensitive duplicate detection', () => {
      ...
      const serviceArbitrary = (source) => fc.record({
        id: baseIdArbitrary,
        name: fc.string({ minLength: 1, maxLength: 30 }),
        iconUrl: fc.constant(null),
        consoleUrl: fc.webUrl({ validSchemes: ['https'] }),
