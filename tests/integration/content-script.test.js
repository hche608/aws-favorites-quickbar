...
const { createAWSConsolePageDOM, createSampleServices } = require('../helpers/fixtures');
const { teardownDOM, waitForCondition } = require('../helpers/dom-helpers');
...
describe('Content Script Integration', () => {
  ...
  describe('Complete injection workflow', () => {
    it('should inject services from user favorites and recent services', async () => {
      ...
      global.chrome.storage.sync.data.userFavorites = userFavorites;
      ...
      const userFavoriteServices = userFavorites.map(id => {
        const recent = recentServices.find(s => s.id.toLowerCase() === id.toLowerCase());
        if (recent) {
      ...
      const mergedServices = window.AWSFavoritesQuickbar.mergeServices(userFavoriteServices, recentServices);
      ...
      const allServiceIds = Array.from(injectedItems).map(item => item.getAttribute('data-service-id'));
  ...
  describe('Background icon update workflow', () => {
    it('should update icons without disrupting quickbar', async () => {
      ...
      const updatedServices = [
        { id: 's3', name: 'S3', iconUrl: 'https://console.aws.amazon.com/s3/new-icon.png', consoleUrl: 'https://console.aws.amazon.com/s3/home', source: 'user' },
        { id: 'ec2', name: 'EC2', iconUrl: 'https://console.aws.amazon.com/ec2/new-icon.png', consoleUrl: 'https://console.aws.amazon.com/ec2/home', source: 'recent' }
      ];
