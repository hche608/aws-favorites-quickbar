...
describe('Popup Workflow Integration Tests', () => {
  ...
  describe('Popup initialization and service loading', () => {
    it('should load and display services from cache', async () => {
      ...
      (global as any).chrome.storage.local.data.cachedServices = {
        services: cachedServices,
        timestamp: Date.now()
      };

      (global as any).chrome.storage.sync.data.userFavorites = ['s3'];

      // Act: Load services using chrome.storage API directly
      const cachedResult = await (chrome as any).storage.local.get(['cachedServices']);
      const favoritesResult = await (chrome as any).storage.sync.get(['userFavorites']);
    ...
    it('should handle empty cache gracefully', async () => {
      ...
      (global as any).chrome.storage.local.data = {};
    ...
    it('should load max services setting', async () => {
      // Arrange
      (global as any).chrome.storage.sync.data.maxServices = 15;

      // Act
      const result = await (chrome as any).storage.sync.get(['maxServices']);
    ...
    it('should use default max services when not set', async () => {
      ...
      (global as any).chrome.storage.sync.data = {};
  ...
  describe('Search interaction filters list in real-time', () => {
    ...
    const searchServices = (query: string, allServices: any[]) => {
      if (!query || query.trim() === '') {
        return allServices;
      }
  ...
  describe('Max services setting updates storage', () => {
    it('should save max services setting', async () => {
      ...
      // Act
      await (chrome as any).storage.sync.set({ maxServices: newValue });

      // Assert
      expect((global as any).chrome.storage.sync.data.maxServices).toBe(20);
    });

    it('should update max services input value', async () => {
      // Arrange
      const maxServicesInput = document.getElementById('maxServicesInput') as HTMLInputElement;
      const newValue = 25;

      // Act
      await (chrome as any).storage.sync.set({ maxServices: newValue });
      const result = await (chrome as any).storage.sync.get(['maxServices']);
      maxServicesInput.value = result.maxServices.toString();
    ...
    it('should handle max services within valid range', async () => {
      ...
      await (chrome as any).storage.sync.set({ maxServices: 1 });
      let result = await (chrome as any).storage.sync.get(['maxServices']);
      expect(result.maxServices).toBe(1);

      await (chrome as any).storage.sync.set({ maxServices: 50 });
      result = await (chrome as any).storage.sync.get(['maxServices']);
      expect(result.maxServices).toBe(50);

      await (chrome as any).storage.sync.set({ maxServices: 10 });
      result = await (chrome as any).storage.sync.get(['maxServices']);
  ...
  describe('UI state management', () => {
    ...
    it('should show error state when requested', () => {
      ...
      expect(errorStateElement.querySelector('.error-message')!.textContent).toBe(
        'Test error message'
      );
    });
  ...
  describe('Favorite management', () => {
    it('should add favorite to storage', async () => {
      ...
      const currentFavorites = (global as any).chrome.storage.sync.data.userFavorites;
      currentFavorites.push('ec2');
      await (chrome as any).storage.sync.set({ userFavorites: currentFavorites });
    ...
    it('should not add duplicate favorites', async () => {
      ...
      const currentFavorites = (global as any).chrome.storage.sync.data.userFavorites;
      const exists = currentFavorites.some((id: string) => id.toLowerCase() === 's3'.toLowerCase());
    ...
    it('should remove favorite from storage', async () => {
      // Arrange
      (global as any).chrome.storage.sync.data.userFavorites = ['s3', 'ec2', 'lambda'];

      // Act: Remove a favorite
      const currentFavorites = (global as any).chrome.storage.sync.data.userFavorites;
      const updated = currentFavorites.filter(
        (id: string) => id.toLowerCase() !== 'ec2'.toLowerCase()
      );
      await (chrome as any).storage.sync.set({ userFavorites: updated });
    ...
    it('should handle case-insensitive favorite operations', async () => {
      ...
      let currentFavorites = (global as any).chrome.storage.sync.data.userFavorites;
      const exists = currentFavorites.some((id: string) => id.toLowerCase() === 'S3'.toLowerCase());
      ...
      let result = await (chrome as any).storage.sync.get(['userFavorites']);
      expect(result.userFavorites.length).toBe(1);

      // Act: Remove with different case
      currentFavorites = result.userFavorites;
      const updated = currentFavorites.filter(
        (id: string) => id.toLowerCase() !== 'S3'.toLowerCase()
      );
      await (chrome as any).storage.sync.set({ userFavorites: updated });

      // Assert: Should remove the favorite
      result = await (chrome as any).storage.sync.get(['userFavorites']);
