  ...
  describe('Property 9: Service pinning toggle idempotence', () => {
    it('should return to original state after pin then unpin', () => {
      const serviceIdArbitrary = fc.stringOf(
        fc.constantFrom('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't'),
      ...
      fc.assert(
        fc.property(
          ...
          (serviceId, initialFavorites) => {
            ...
            const pinnedAfterAdd = currentFavorites.some(
              id => id === serviceId.toLowerCase()
            );
            ...
            const pinnedAfterRemove = currentFavorites.some(
              id => id === serviceId.toLowerCase()
            );
      ...
      fc.assert(
        fc.property(
          ...
          (serviceId, initialFavorites, cycles) => {
            ...
            const finallyPinned = currentFavorites.some(
              id => id === serviceId.toLowerCase()
            );
            ...
            if (initialHadService) {
              return !finallyPinned && currentFavorites.length === uniqueInitialFavorites.length - 1;
            } else {
      ...
      fc.assert(
        fc.property(
          ...
          (serviceId, initialFavorites) => {
            ...
            const mixedServiceId = serviceId.split('').map((c, i) => 
              i % 2 === 0 ? c.toLowerCase() : c.toUpperCase()
            ).join('');
            currentFavorites = currentFavorites.filter(
              id => id !== mixedServiceId.toLowerCase()
            );
      ...
      fc.assert(
        fc.property(
          ...
          (serviceId, otherFavorites) => {
            ...
            const allOthersPresent = uniqueOtherFavorites.every(
              otherId => currentFavorites.some(id => id === otherId)
            );
            ...
            const stillAllOthersPresent = uniqueOtherFavorites.every(
              otherId => currentFavorites.some(id => id === otherId)
            );
