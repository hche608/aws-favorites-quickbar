        ...
        fc.property(
          ...
          (favorites, fromIndex, toIndex) => {
            ...
            const uniqueFavorites = Array.from(
              new Set(favorites.map(id => id.toLowerCase()))
            );
            ...
            global.chrome.storage.sync.data.userFavorites = reordered;

            // Verify the order is persisted
            const storedFavorites = global.chrome.storage.sync.data.userFavorites;
      ...
      fc.assert(
        fc.property(
          ...
          (favorites, fromIndex, toIndex) => {
            ...
            const allPresent = uniqueFavorites.every(
              id => reordered.includes(id)
            );

            // Property: No new favorites should be added
            const noNewItems = reordered.every(
              id => uniqueFavorites.includes(id)
            );
      ...
      fc.assert(
        fc.property(
          ...
          (favorites, swapIndex) => {
            ...
            const swapped = reordered[normalizedIndex] === item2 && 
                           reordered[normalizedIndex + 1] === item1;
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (favorites, fromIndex, toIndex) => {
            ...
            // Save to storage
            await chrome.storage.sync.set({ userFavorites: reordered });
